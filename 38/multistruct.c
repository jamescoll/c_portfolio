/* multistruct.c - a program that contains a list
...
student *add_record(student *record);
...
int main(void)
{
	...
	//load data from file linkedlist.dat into linkedlist 
...
student *load_all(student *start)
{
	...
	//the last line twice
	while(fscanf(fp, "%s %s %d", &start->name, &start->phone_number, &start->student_number)!=EOF)
	{	
...
void save_all(student *p)
{
	...
	for (; p != NULL; p = p->next)
	{
		...
		fprintf(fp, "%s %s %d\n", &p->name, &p->phone_number, &p->student_number);
